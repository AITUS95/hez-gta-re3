...
#undef FixStoredCar
	...
	memset(buf2, 0, NUM_CORLEONE_GARAGE_STORED_CARS * sizeof(CStoredCar));
	buf2 += NUM_CORLEONE_GARAGE_STORED_CARS * sizeof(CStoredCar);
	written += NUM_CORLEONE_GARAGE_STORED_CARS * sizeof(CStoredCar);
	...
	*size = (6 * sizeof(uint32) + TOTAL_COLLECTCARS_GARAGES * sizeof(*CGarages::CarTypesCollected) + sizeof(uint32) + 3 * NUM_GARAGE_STORED_CARS * sizeof(CStoredCar) + NUM_GARAGES * sizeof(CGarage) + NUM_CORLEONE_GARAGE_STORED_CARS * sizeof(CStoredCar));
#else
	*size = 5484;
#endif
}
