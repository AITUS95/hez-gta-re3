...
#define LIMIT_CARS_IN_COLOMBIAN (MAX_COLOMBIAN_GARAGE_STORED_CARS)
...
CStoredCar CGarages::aCarsInColombianGarage[NUM_COLOMBIAN_GARAGE_STORED_CARS];
...
void CGarages::Init(void)
{
	...
	for (int i = 0; i < NUM_COLOMBIAN_GARAGE_STORED_CARS; i++)
		aCarsInColombianGarage[i].Init();
	...
	case GARAGE_HIDEOUT_COLOMBIAN:
		...
		case GS_CLOSING:
			...
			if (!IsPlayerOutsideGarage())
				...
				case GARAGE_HIDEOUT_COLOMBIAN: StoreAndRemoveCarsForThisHideout(CGarages::aCarsInColombianGarage, MAX_COLOMBIAN_GARAGE_STORED_CARS); break;
				default: break;
				}
			}
			UpdateDoorsHeight();
		...
		case GS_FULLYCLOSED:
		{
			...
			if (distance < SQR(DISTANCE_TO_OPEN_HIDEOUT_GARAGE_ON_FOOT) ||
				...
				if (FindPlayerVehicle() && CGarages::CountCarsInHideoutGarage(m_eGarageType) >= CGarages::FindMaxNumStoredCarsForGarage(m_eGarageType)) {
					...
					case GARAGE_HIDEOUT_COLOMBIAN: bCreatedAllCars = RestoreCarsForThisHideout(CGarages::aCarsInColombianGarage); break;
					default: break;
					}
					if (bCreatedAllCars)
						m_eGarageState = GS_OPENING;
...
void CGarages::CloseHideOutGaragesBeforeSave()
{
	for (int i = 0; i < NUM_GARAGES; i++) {
		if (aGarages[i].m_eGarageType != GARAGE_HIDEOUT_ONE &&
			...
			aGarages[i].m_eGarageType != GARAGE_HIDEOUT_COLOMBIAN)
			continue;
		if (aGarages[i].m_eGarageState != GS_FULLYCLOSED &&
			...
			case GARAGE_HIDEOUT_THREE:
				...
				break;
			case GARAGE_HIDEOUT_COLOMBIAN:
				aGarages[i].StoreAndRemoveCarsForThisHideout(aCarsInColombianGarage, MAX_COLOMBIAN_GARAGE_STORED_CARS);
				aGarages[i].RemoveCarsBlockingDoorNotInside();
				break;
			default:
				break;
...
int32 CGarages::CountCarsInHideoutGarage(uint8 type)
{
	...
	if (type == GARAGE_HIDEOUT_COLOMBIAN) {
		total = 0;
		for (int i = 0; i < NUM_COLOMBIAN_GARAGE_STORED_CARS; i++)
			total += aCarsInColombianGarage[i].HasCar();
	}
	return total;
}
...
int32 CGarages::FindMaxNumStoredCarsForGarage(uint8 type)
{
	...
	case GARAGE_HIDEOUT_COLOMBIAN:
		return LIMIT_CARS_IN_COLOMBIAN;
	default: break;
	}
	return 0;
}
...
void CGarages::Save(uint8 * buf, uint32 * size)
{
	...
	INITSAVEBUF
	*size = (6 * sizeof(uint32) + TOTAL_COLLECTCARS_GARAGES * sizeof(*CarTypesCollected) + sizeof(uint32) + 3 * NUM_GARAGE_STORED_CARS * sizeof(CStoredCar) + NUM_GARAGES * sizeof(CGarage) + NUM_COLOMBIAN_GARAGE_STORED_CARS * sizeof(CStoredCar));
#else
	* size = 5484;
#endif
	...
	for (int i = 0; i < NUM_COLOMBIAN_GARAGE_STORED_CARS; i++)
		WriteSaveBuf(buf, aCarsInColombianGarage[i]);
#ifdef FIX_GARAGE_SIZE
...
void CGarages::Load(uint8* buf, uint32 size)
{
#ifdef FIX_GARAGE_SIZE
	INITSAVEBUF
	assert(size == (6 * sizeof(uint32) + TOTAL_COLLECTCARS_GARAGES * sizeof(*CarTypesCollected) + sizeof(uint32) + 3 * NUM_GARAGE_STORED_CARS * sizeof(CStoredCar) + NUM_GARAGES * sizeof(CGarage) + NUM_COLOMBIAN_GARAGE_STORED_CARS * sizeof(CStoredCar)));
#else
	assert(size == 5484);
#endif
	CloseHideOutGaragesBeforeSave();
	...
	for (int i = 0; i < NUM_COLOMBIAN_GARAGE_STORED_CARS; i++)
		ReadSaveBuf(&aCarsInColombianGarage[i], buf);
#ifdef FIX_GARAGE_SIZE
	VALIDATESAVEBUF(size);
#endif
